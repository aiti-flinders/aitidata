update_cabee_sa2 <- function() {
  ...
  current_date <- readxl::read_excel(abs_test,
                                     ...
                                     range = "A2",
                                     col_names = "release") %>%
    dplyr::mutate(release = stringr::str_sub(release, start = -9L, end = -1L),
  ...
  if (current_date <= max(aitidata::cabee_sa2$date)) {
    ...
    for (i in cabee_sheets) {
      cabee_year <- readxl::read_excel(abs_file,
        ...
        dplyr::filter(!is.na(sa2_main_2016)) %>%
        dplyr::mutate(sa2_main_2016 = as.character(sa2_main_2016),
