...
update_internet_vacancies_regional <- function(force_update = FALSE) {
  ...
  if (!force_update) {
    ...
    dl <- httr::GET(
      url = "https://www.jobsandskills.gov.au/sites/default/files/2022-11/IVI_DATA_SkillLevel%20-%20January%202006%20onwards.xlsx",
  ...
  if (current_date > max(aitidata::internet_vacancies_regional$date) | force_update) {
    ...
    dl <- httr::GET(
      url = "https://www.jobsandskills.gov.au/sites/default/files/2022-11/IVI_DATA_regional%20-%20May%202010%20onwards.xlsx",
      header = httr::add_headers(header),
      httr::write_disk(here::here("data-raw/ivi_regional.xlsx"), overwrite = TRUE)
      )
