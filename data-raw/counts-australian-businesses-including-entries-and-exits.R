...
download_abs_data_cube("counts-australian-businesses-including-entries-and-exits", cube = "816508", path = "data-raw")
...
cabee_sa2 <- cabee_sa2 %>%
  pivot_longer(
    cols = c(-industry_code, -industry_label, -sa2_main_2016, -sa2_name_2016, -date),
    names_to = "indicator",
    values_to = "value"
  ) %>%
  select(date,
