...
industry_value_add <- ana %>%
  ...
  filter(industry %in% daitir::anzsic$division,
         !is.na(value)) %>% 
  ...
  mutate(
    ...
    subdivision = ifelse(subdivision %in% c("Gross Value Added", 
                                            "Percentage Changes",
...
key_agg <- read_abs("5206.0", tables = 1)

industry_aggregates <- key_agg %>%
...
usethis::use_data(national_accounts, compress = "xz", overwrite = TRUE)
